...
func Part2() {
	...
	*/
	...
	for gId, game := range games {
		...
		for setId := 0; setId < len(game); setId++ {
			set := game[setId]
			fmt.Println(set)
		...
		powerOfMinSet = minRed * minGreen * minBlue

		fmt.Printf("MIN_RED: %d \t MIN_GREEN: %d \t MIN_BLUE: %d \n ", minRed, minGreen, minBlue)
		fmt.Println("-----------------------")
		fmt.Println(" \t Power of the minimum set of cubes \t  \n ", powerOfMinSet)
		fmt.Println("-----------------------")
		fmt.Printf("RED: %d \t GREEN: %d \t BLUE: %d \n ", colors["red"], colors["green"], colors["blue"])
